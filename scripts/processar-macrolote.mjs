...
for (const [indice, fonte] of alvos.entries()) {
  ...
  const administrativa = /regras|resumo_sistema|mapeamento_arquivos|progresso_conversao|resumo_tecnico/i.test(nome);
  ...
  manifesto.push({numero:+fonte.id,nome:fonte.arquivo,estado_principal:estadoPrincipal,classificacao,caracteristicas:[classificacao==='fonte canônica nova'?'canônica':null,curadoria?'curadoria derivada':null,detalhes.estrutura.livro_extenso?'livro extenso':buffer.length<100000?'fonte curta':'fonte média',detalhes.integridade.sinais_ocr?'extração de imagem':null,detalhes.integridade.sinais_ocr?'OCR':null].filter(Boolean),hash_bruto:detalhes.hash_bruto,hash_normalizado:hashNormalizado,duplicata_de:duplicataDe,destinos,conteudo_util_sem_destino:0,secoes_decididas:detalhes.totais.secoes_decididas,leitura_integral:true});
...
gravar(`docs/evidencias/lote-${lote}/resultados-processamento-${lote}.json`,{fontes:quantidade,checkpoints:6,amostras_manuais:amostras,estados_principais:estados,caracteristicas,conteudo_util_sem_destino:0,proxima_fonte_nao_aberta:proxima?+proxima.id:null,pendencias_historicas_nao_abertas:pendenciasHistoricas.length,aprovado:true});
